...
export const updateDocument = (document) => {
    return new Promise((resolve, reject) => {
        db.run("UPDATE Document SET title=?, description=?, scale=?, ASvalue=?, issuanceDate=?, type=?, connections=?, language=?, pages=? WHERE docId=?",
            [
                document.title,
                document.description,
                document.scale,
...
export const updateDocumentStakeholder = (docId, stakeholdersId) => {
    return new Promise((resolve, reject) => {
        db.run("DELETE FROM DocStakeholders WHERE docId=?", [docId], (err) => {
            if (err) {
                reject(err);
            } else {
                stakeholdersId.forEach(shId => {
                    db.run("INSERT INTO DocStakeholders (docId, shId) VALUES (?,?)", [docId, shId], (err, rows) => {
                        if (err) {
                            reject(err)
                        } else {
                            resolve("Update  document stakeholder done ")
                        }
                    })
                });
            }
